...
*** Commutative RDTs
    ...
    * Commutative merge operation (~f(x, y) = f(y, x)~)
    * Not necessarily idempotent (~f(x, y) != f(f(x, y))~)
...
*** CRDTs in practice
    ...
    Usually convergent, not commutative

    Solve a hard algorithmic problem once
...
*** Using CRDTs
    ...
    * Designing them is fairly tricky
